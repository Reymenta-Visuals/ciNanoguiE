                                ...
                                int modifiers)
{
   ...
   if (mEditable && focused() && button == MOUSE_BUTTON_LEFT)
   {
      if (down)
      {
         ...
         double time = NanoUtil::getElapsedSeconds();
...
bool TextBox::copySelection()
{
   if (mSelectionPos > -1)
   {
      ...
      if (begin > end)
         std::swap (begin, end);
      cinder::Clipboard::setString (mValueTemp.substr (begin, end).c_str());
                            ...
                            const NVGglyphPosition * glyphs, int size)
{
   ...
   {
      if (mMouseDownModifier == SHIFT_DOWN)
      {
         if (mSelectionPos == -1)
            mSelectionPos = mCursorPos;
      }
